...
var PlatformState = function () {
    ...
    _createClass(PlatformState, [{
        ...
        value: function renderToString() {
            return ɵgetDOM().getInnerHTML(this._doc);
        }
        /**
...
PlatformState.decorators = [{ type: Injectable }];
/** @nocollapse */
...
ServerXhr.decorators = [{ type: Injectable }];
/** @nocollapse */
...
ServerXsrfStrategy.decorators = [{ type: Injectable }];
/** @nocollapse */
...
ServerPlatformLocation.decorators = [{ type: Injectable }];
/** @nocollapse */
...
ServerRootRenderer.decorators = [{ type: Injectable }];
/** @nocollapse */
ServerRootRenderer.ctorParameters = function () {
    return [{ type: undefined, decorators: [{ type: Inject, args: [DOCUMENT] }] }, { type: ɵSharedStylesHost }, { type: AnimationDriver }, { type: undefined, decorators: [{ type: Inject, args: [APP_ID] }] }, { type: NgZone }];
};
...
var ServerRenderer = function () {
    ...
    function ServerRenderer(_rootRenderer, componentProto, _animationDriver, styleShimId, _zone, _schema) {
        ...
        this._styles = ɵflattenStyles(styleShimId, componentProto.styles, []);
        if (componentProto.encapsulation === ViewEncapsulation.Native) {
            throw new Error('Native encapsulation is not supported on the server!');
        }
        this._rootRenderer.sharedStylesHost.addStyles(this._styles);
        if (this.componentProto.encapsulation === ViewEncapsulation.Emulated) {
            this._contentAttr = ɵshimContentAttribute(styleShimId);
            this._hostAttr = ɵshimHostAttribute(styleShimId);
        } else {
    ...
    _createClass(ServerRenderer, [{
        ...
        value: function selectRootElement(selectorOrNode, debugInfo) {
            ...
            if (typeof selectorOrNode === 'string') {
                el = ɵgetDOM().querySelector(this._rootRenderer.document, selectorOrNode);
                if (isBlank(el)) {
        ...
        value: function createElement(parent, name, debugInfo) {
            var /** @type {?} */el = void 0;
            if (ɵisNamespaced(name)) {
                var /** @type {?} */nsAndName = ɵsplitNamespace(name);
                el = ɵgetDOM().createElementNS(ɵNAMESPACE_URIS[nsAndName[0]], nsAndName[1]);
            } else {
                el = ɵgetDOM().createElement(name);
            }
            if (isPresent(this._contentAttr)) {
                ɵgetDOM().setAttribute(el, this._contentAttr, '');
            }
            if (isPresent(parent)) {
                ɵgetDOM().appendChild(parent, el);
            }
            return el;
        }
        /**
        ...
        value: function createViewRoot(hostElement) {
            ...
            if (isPresent(this._hostAttr)) {
                ɵgetDOM().setAttribute(hostElement, this._hostAttr, '');
            }
        ...
        value: function createTemplateAnchor(parentElement, debugInfo) {
            var /** @type {?} */comment = ɵgetDOM().createComment(TEMPLATE_COMMENT_TEXT);
            if (isPresent(parentElement)) {
                ɵgetDOM().appendChild(parentElement, comment);
            }
            return comment;
        }
        /**
        ...
        value: function createText(parentElement, value, debugInfo) {
            var /** @type {?} */node = ɵgetDOM().createTextNode(value);
            if (isPresent(parentElement)) {
                ɵgetDOM().appendChild(parentElement, node);
            }
            return node;
        }
        /**
        ...
        value: function detachView(viewRootNodes) {
            for (var /** @type {?} */i = 0; i < viewRootNodes.length; i++) {
                ɵgetDOM().remove(viewRootNodes[i]);
            }
        }
        /**
         * @param {?} hostElement
        ...
        value: function listen(renderElement, name, callback) {
            ...
            return this._zone.runOutsideAngular(function () {
                return ɵgetDOM().onAndCancel(renderElement, name, outsideHandler);
            });
        }
        /**
         * @param {?} target
        ...
        value: function listenGlobal(target, name, callback) {
            var /** @type {?} */renderElement = ɵgetDOM().getGlobalEventTarget(this._rootRenderer.document, target);
        ...
        value: function setElementProperty(renderElement, propertyName, propertyValue) {
            ɵgetDOM().setProperty(renderElement, propertyName, propertyValue);
        ...
        value: function setElementAttribute(renderElement, attributeName, attributeValue) {
            ...
            if (ɵisNamespaced(attributeName)) {
                var /** @type {?} */nsAndName = ɵsplitNamespace(attributeName);
                attrNameWithoutNs = nsAndName[1];
                attributeName = nsAndName[0] + ':' + nsAndName[1];
                attrNs = ɵNAMESPACE_URIS[nsAndName[0]];
            }
            if (isPresent(attributeValue)) {
                if (isPresent(attrNs)) {
                    ɵgetDOM().setAttributeNS(renderElement, attrNs, attributeName, attributeValue);
                } else {
                    ɵgetDOM().setAttribute(renderElement, attributeName, attributeValue);
                }
            } else {
                if (isPresent(attrNs)) {
                    ɵgetDOM().removeAttributeNS(renderElement, attrNs, attrNameWithoutNs);
                } else {
                    ɵgetDOM().removeAttribute(renderElement, attributeName);
                }
            }
        }
        /**
        ...
        value: function setBindingDebugInfo(renderElement, propertyName, propertyValue) {
            if (ɵgetDOM().isCommentNode(renderElement)) {
                var /** @type {?} */existingBindings = ɵgetDOM().getText(renderElement).replace(/\n/g, '').match(TEMPLATE_BINDINGS_EXP);
                var /** @type {?} */parsedBindings = JSON.parse(existingBindings[1]);
                parsedBindings[/** TODO #9100 */propertyName] = propertyValue;
                ɵgetDOM().setText(renderElement, TEMPLATE_COMMENT_TEXT.replace('{}', JSON.stringify(parsedBindings, null, 2)));
            } else {
        ...
        value: function setElementClass(renderElement, className, isAdd) {
            if (isAdd) {
                ɵgetDOM().addClass(renderElement, className);
            } else {
                ɵgetDOM().removeClass(renderElement, className);
            }
        }
        /**
         * @param {?} renderElement
        ...
        value: function setElementStyle(renderElement, styleName, styleValue) {
            if (isPresent(styleValue)) {
                ɵgetDOM().setStyle(renderElement, styleName, stringify(styleValue));
            } else {
                ɵgetDOM().removeStyle(renderElement, styleName);
            }
        }
        /**
         * @param {?} renderElement
        ...
        value: function invokeElementMethod(renderElement, methodName, args) {
            ɵgetDOM().invoke(renderElement, methodName, args);
        }
        /**
         * @param {?} renderNode
         * @param {?} text
        ...
        value: function setText(renderNode, text) {
            ɵgetDOM().setText(renderNode, text);
        }
        /**
         * @param {?} element
...
function moveNodesAfterSibling(ref, nodes) {
    var /** @type {?} */parent = ɵgetDOM().parentElement(ref);
    if (nodes.length > 0 && parent) {
        var /** @type {?} */nextSibling = ɵgetDOM().nextSibling(ref);
        if (nextSibling) {
            for (var /** @type {?} */i = 0; i < nodes.length; i++) {
                ɵgetDOM().insertBefore(parent, nextSibling, nodes[i]);
            }
        } else {
            for (var /** @type {?} */_i = 0; _i < nodes.length; _i++) {
                ɵgetDOM().appendChild(parent, nodes[_i]);
            }
        }
    }
}
...
function appendNodes(parent, nodes) {
    for (var /** @type {?} */i = 0; i < nodes.length; i++) {
        ɵgetDOM().appendChild(parent, nodes[i]);
    }
}
...
var DefaultServerRendererV2 = function () {
    ...
    _createClass(DefaultServerRendererV2, [{
        ...
        value: function createComment(value, debugInfo) {
            return ɵgetDOM().createComment(value);
        }
        /**
         * @param {?} value
         * @param {?=} debugInfo
        ...
        value: function createText(value, debugInfo) {
            return ɵgetDOM().createTextNode(value);
        }
        /**
         * @param {?} parent
         * @param {?} newChild
        ...
        value: function appendChild(parent, newChild) {
            ɵgetDOM().appendChild(parent, newChild);
        }
        /**
         * @param {?} parent
         * @param {?} newChild
        ...
        value: function parentNode(node) {
            return ɵgetDOM().parentElement(node);
        }
        /**
         * @param {?} node
         * @return {?}
        ...
        value: function nextSibling(node) {
            return ɵgetDOM().nextSibling(node);
        }
        /**
         * @param {?} el
         * @param {?} name
        ...
        value: function listen(target, eventName, callback) {
            ...
            return this.ngZone.runOutsideAngular(function () {
                return ɵgetDOM().onAndCancel(el, eventName, outsideHandler);
            });
        }
    }]);
...
var EmulatedEncapsulationServerRendererV2 = function (_DefaultServerRendere) {
    ...
    function EmulatedEncapsulationServerRendererV2(document, ngZone, sharedStylesHost, schema, component) {
        ...
        _this8.contentAttr = ɵshimContentAttribute(component.id);
        _this8.hostAttr = ɵshimHostAttribute(component.id);
        return _this8;
    }
    /**
     * @param {?} element
...
ServerModule.decorators = [{ type: NgModule, args: [{
