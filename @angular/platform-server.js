...
class ServerRenderer {
    ...
    createElement(parent, name, debugInfo) {
        ...
        if (ɵisNamespaced(name)) {
            const /** @type {?} */ nsAndName = ɵsplitNamespace(name);
    ...
    createTemplateAnchor(parentElement, debugInfo) {
        const /** @type {?} */ comment = ɵgetDOM().createComment(TEMPLATE_COMMENT_TEXT);
    ...
    createText(parentElement, value, debugInfo) {
        const /** @type {?} */ node = ɵgetDOM().createTextNode(value);
        if (isPresent(parentElement)) {
    ...
    listen(renderElement, name, callback) {
        ...
        return this._zone.runOutsideAngular(() => ɵgetDOM().onAndCancel(renderElement, name, outsideHandler));
    }
    /**
     * @param {?} target
     * @param {?} name
    ...
    listenGlobal(target, name, callback) {
        const /** @type {?} */ renderElement = ɵgetDOM().getGlobalEventTarget(this._rootRenderer.document, target);
    ...
    setElementAttribute(renderElement, attributeName, attributeValue) {
        ...
        if (ɵisNamespaced(attributeName)) {
            const /** @type {?} */ nsAndName = ɵsplitNamespace(attributeName);
    ...
    setBindingDebugInfo(renderElement, propertyName, propertyValue) {
        if (ɵgetDOM().isCommentNode(renderElement)) {
            const /** @type {?} */ existingBindings = ɵgetDOM().getText(renderElement).replace(/\n/g, '').match(TEMPLATE_BINDINGS_EXP);
     ...
     * @return {?}
     */
    setText(renderNode, text) { ɵgetDOM().setText(renderNode, text); }
    /**
     * @param {?} element
...
function moveNodesAfterSibling(ref, nodes) {
    const /** @type {?} */ parent = ɵgetDOM().parentElement(ref);
    if (nodes.length > 0 && parent) {
        const /** @type {?} */ nextSibling = ɵgetDOM().nextSibling(ref);
        if (nextSibling) {
